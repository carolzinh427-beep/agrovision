  ...
  return (
    <section className="py-12 sm:py-16 md:py-20 bg-white border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 3 Pillars - Side-by-side horizontal row on mobile */}
        <div className="flex overflow-x-auto gap-3.5 pb-4 pt-1 snap-x snap-mandatory md:grid md:grid-cols-3 md:gap-6 md:overflow-visible scrollbar-none">
          {pillars.map((pillar, idx) => {
            const IconComp = pillar.icon;
            return (
              <div
                key={idx}
                className="flex-none w-[82%] sm:w-[70%] md:w-auto snap-center bg-white p-5 sm:p-6 border border-slate-200 hover:border-slate-400 transition-all duration-150 relative flex flex-col justify-between rounded-none shadow-xs group"
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-11 h-11 bg-slate-100 border border-slate-200 flex items-center justify-center text-[#15803D]">
                  ...
                  <h3 className="text-base sm:text-lg font-extrabold text-slate-900 font-['Outfit'] mb-1 group-hover:text-[#15803D] transition-colors">
                    {pillar.title}
                  </h3>
